...
from .catelog import DatasetCatalog
...
def build_dataset(cfg, logger, dataset_list, transforms, dataset_catalog, is_train=True, class_concat=False, extra_args={}):
    """
    Arguments:
    ...
    for dataset_id, dataset_name in enumerate(dataset_list, 1):
        ...
        if data["factory"] in ["CocoGrounding", "CocoDetectionTSV", "CaptionTSV", "MixedDataset", "FlickrDataset", "RefExpDataset", "GQADataset", "PseudoData", "PhrasecutDetection"]:
            ...
            args["max_num_labels"] = -1
            args["max_query_len"] = 256
        ...
        logger.info(f"{dataset_name} is wrapped in {data['factory']} class, found in total {len(dataset)} data points.")
        ...
        if class_concat:
            ...
            logger.info("Found {} #category after group {}, concating ...".format(num_category, dataset_id))
...
def make_data_loader(cfg, logger, is_train=True, is_distributed=False, num_replicas=None, rank=None, start_iter=0):
    ...
    logger.info("The combined datasets are: {}.".format(dataset_list))
    ...
    if is_train and cfg.DATASETS.RANDOM_SAMPLE_NEG > 0:
        extra_args['random_sample_neg'] = cfg.DATASETS.RANDOM_SAMPLE_NEG
    ...
    if is_train:
        extra_args["local_debug"] = False
    if is_train:
        extra_args["no_mask_for_od"] = False
    if is_train:
        extra_args["no_mask_for_gold"] = False
    if is_train:
        extra_args["mlm_obj_for_only_positive"] = False
    ...
    if is_train:
        extra_args["special_safeguard_for_coco_grounding"] = False
    if is_train:
        extra_args["diver_box_for_vqa"] = False
    ...
    if cfg.MODEL.tokenizer_type == "clip":
        ...
        extra_args['tokenizer'] = AutoTokenizer.from_pretrained(cfg.MODEL.tokenizer_type)
    ...
    if isinstance(dataset_list[0], (tuple, list)):
        ...
        datasets = build_dataset(cfg, logger, dataset_list, transforms, dataset_catalog, is_train,
                                 class_concat=cfg.DATASETS.CLASS_CONCAT,
                                 extra_args=extra_args)
