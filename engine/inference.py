...
from models.GroundingDINO.groundingdino import PostProcess
from data.datasets.evaluation import evaluate, im_detect_bbox_aug
from util.misc import is_main_process, all_gather, synchronize, get_world_size
from util.utils import get_phrases_from_posmap
from data.datasets.evaluation.flickr.flickr_eval import FlickrEvaluator
from data.structures.bounding_box import BoxList
import matplotlib.pyplot as plt
from data.datasets.tsv import load_from_yaml_file
...
def inference_default(
    ...
    for i, batch in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            if cfg.TEST.USE_MULTISCALE:
                assert cfg.TEST.USE_MULTISCALE == False, "Currently not support multiscale testing."
    ...
    predictions = _accumulate_predictions_from_multiple_gpus(logger, predictions)
...
def create_positive_dict(tokenized, tokens_positive, labels):
    ...
    # token to label and label to token
...
def create_queries_and_maps_from_dataset(model, dataset, cfg):
    '''
    1. read category from datasets, assume already sorted
    2. initialize 2 lists
        a. labels: contain the <int> index for each category
        b. label_list: contain the <str> name for each category
    3. decide whether or not to chunk the category lists
        a. if chunk is enabled, divide the category into n_cats / n chunks
    4. labels: List(chunks); label_list: List(chunks)
    5. create 2 lists, however, the tokenized squence is not returned in this function.
        a. all_queries: List(each query pieces with separation and text prompts)
        b. all_positive_map_label_to_token: List(maps from label to token index)
    '''
    ...
    for i in range(len(labels)):
        ...
        query_i, positive_map_label_to_token_i = create_queries_and_maps(model,
...
def create_queries_and_maps(model, labels, label_list, additional_labels = None, cfg = None):
    ...
    # this part only look to generate correspondence, so it's okay to use pretrained tokenizer.
...
def _accumulate_predictions_from_multiple_gpus(logger, predictions_per_gpu):
...
def build_lvis_evaluator(ann_file, fixed_ap=True):
    from data.datasets.evaluation.lvis.lvis import LVIS
    from data.datasets.evaluation.lvis.lvis_eval import LvisEvaluatorFixedAP, LvisEvaluator
...
def do_inference(
    ...
    if not task:
        return inference_default(logger,
            model, data_loader, dataset_name, 
            iou_types, box_only, device, expected_results, 
            expected_results_sigma_tol, output_folder, cfg)
    ...
    if task == "detection":
        all_queries, all_positive_map_label_to_token = create_queries_and_maps_from_dataset(model, dataset, cfg)
    elif task == "grounding":
        # need captions for inputs 
        all_queries = [None]
    ...
    for i, batch in enumerate(_iterator):
        ...
        with torch.no_grad():
            if cfg.TEST.USE_MULTISCALE:
                assert cfg.TEST.USE_MULTISCALE == False, "Multiscale testing is currently not supported."
                # number of chunks, query_time
                ...
                for query_i in range(query_time):
                    ...
                    '''
                    different datasets seem to have different data format... 
                    For LVIS dataset, the target is a dictionary, 
                    while for modulatedDataset such as COCO/Flickr, the target is a BoxList
                    '''
                    if task == "detection":
                        captions = [all_queries[query_i] for ii in range(len(targets))]
                        positive_map_label_to_token = all_positive_map_label_to_token[query_i]
                    ...
                    '''
                    ''' 
                    TODO: add postprocessor to achieve the following functionality:
                        process the pred_logits and pred_bboxes fields into actual box prediction and corresponding labels
                        
                        1. add support for chunk evaluations. (check)
                        2. add support for batch inference.   (check)
                    '''

                    outputs = postprocessor(
                        outputs, positive_map_label_to_token, 
                        images.image_sizes, chunk_size=len(positive_map_label_to_token), test=True)

                    output = [o.to(cpu_device) for o in outputs]

                    # TODO: add support for grounding tasks such as flicker
                    # TODO: add support for LVIS evaluation
                    if "flickr" in cfg.DATASETS.TEST[0]:
                        output = output[0]
        ...
        if evaluator is not None:
            ...
            output = [[chk[bi] for chk in all_output] for bi in range(len(all_output[0]))]
    ...
    if evaluator is not None:
        ...
        try:
            ...
            logger.info("Evaluator has no accumulation, skipped...")
        ...
        try:
            ...
            logger.warning("Default process group is not initialized")
        return
    ...
    predictions = _accumulate_predictions_from_multiple_gpus(logger, predictions)
    logger.info("Accumulated results...")
    if not is_main_process():
        return None
